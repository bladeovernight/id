...
matrix = np.load("2 work/task1/matrix_38.npy")
...
with open("matrix_38.json", "w") as result:
    result.write(json.dumps(matrix_stat))

good_matrix = np.ndarray((size, size), dtype=float)

for i in range(0, size):
    for j in range(0, size):
        good_matrix[i][j] = matrix[i][j] / matrix_stat["sum"]

np.save("good_matrix", good_matrix)