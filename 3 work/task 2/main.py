...
items = []
for i in range(1, 26):
    file_name = f"3 work/task 2/2/{i}.html"
    items += handler(file_name)

with open("3 work/task 2/result/2_result_all.json", 'w', encoding='utf-8') as file:
    file.write(json.dumps(sorted(items, key=lambda x: x['price'], reverse=True), ensure_ascii=False, indent=2))

with open('3 work/task 2/result/2_filtered_result.json', 'w', encoding='utf-8') as json_file:
    filtered_items = []
    for phone in items:
