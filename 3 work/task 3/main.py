...
for i in range(1, 500):
    file_name = f"3 work/task 3/3/{i}.xml"
    items.append(handler(file_name))

with open("3 work/task 3/result/3_result_all.json", 'w', encoding='utf-8') as json_file:
    json_file.write(json.dumps(sorted(items, key=lambda x: x['radius'], reverse=True), ensure_ascii=False, indent=2))

filtered_items = [item for item in items if item['constellation'] == 'Овен']
with open('3 work/task 3/result/3_filtered_result.json', 'w', encoding='utf-8') as json_file:
...
with open('3 work/task 3/result/3_stats.json', 'w', encoding='utf-8') as json_file:
