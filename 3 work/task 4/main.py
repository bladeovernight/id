...
for i in range(1, 100):
    file_name = f"3 work/task 4/4/{i}.xml"
    items += handler(file_name)

with open("3 work/task 4/result/4_result_all.json", 'w', encoding='utf-8') as json_file:
    json_file.write(json.dumps(sorted(items, key=lambda x: x['price'], reverse=True), ensure_ascii=False, indent=2))

with open('3 work/task 4/result/4_filtered_result.json', 'w', encoding='utf-8') as json_file:
    filtered_items = []
    for cloth in items:
...
with open('3 work/task 4/result/4_stats.json', 'w', encoding='utf-8') as json_file:
